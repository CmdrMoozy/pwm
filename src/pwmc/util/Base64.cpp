...
const std::map<char, uint8_t> DECODE_MAP{{'A', 0x00_b},
                                         {'B', 0x01_b},
                                         {'C', 0x02_b},
                                         {'D', 0x03_b},
                                         {'E', 0x04_b},
...
const std::map<uint8_t, char> ENCODE_MAP{{0x00_b, 'A'},
                                         {0x01_b, 'B'},
                                         {0x02_b, 'C'},
                                         {0x03_b, 'D'},
                                         {0x04_b, 'E'},
...
std::vector<uint8_t> decodeBase64(std::string const &s)
{
	...
	for(std::size_t i = 0; (i + 3) < stripped.length(); i += 4)
	{
		...
		ret[retOffset] = av;
		if(stripped[i + 2] != '=') ret[retOffset + 1] = bv;
		if(stripped[i + 3] != '=') ret[retOffset + 2] = cv;
	}
